...
from adjuv_functions.ml_deep.utils import timer, saveModel, loadDNN, validation, validation_multiclass
from adjuv_functions.ml_deep.parameters_deep import param_deep
from adjuv_functions.ml_deep.plot_curves_ml_propythia import plot_roc_curve, plot_precision_recall_curve, \
...
# ,
    ...
    def train_model_cv(self, x_cv, y_cv, cv=5, model=None):
        ...
        for j, (train_idx, val_idx) in enumerate(folds):
            ...
            self.x_train, self.x_dval, self.y_train, self.y_dval = \
                train_test_split(x_train, y_train, test_size=self.validation_split, random_state=42, shuffle=True, stratify=y_train)
            self.x_train, self.x_test, self.x_dval = map(lambda x: np.array(x, dtype=np.int8), [self.x_train, self.x_test, self.x_dval])
            self.generate_callbacks()
